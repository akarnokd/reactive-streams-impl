...
import com.github.akarnokd.rs.impl.queue.*;
 ...
 * 
 */
public final class FlatMap<T, U> implements Publisher<U> {
    ...
    public FlatMap(Publisher<? extends T> source, 
    ...
    public void subscribe(Subscriber<? super U> s) {
        source.subscribe(new MergeSubscriber<>(s, mapper, delayErrors, maxConcurrency, bufferSize));
    }
    ...
    static final class MergeSubscriber<T, U> extends AtomicInteger implements Subscription, Subscriber<T> {
        ...
        public void onNext(T t) {
            ...
            if (p instanceof ScalarSource) {
                tryEmitScalar(((ScalarSource<? extends U>)p).value());
            } else {
    ...
    static final class InnerSubscriber<T, U> implements Subscriber<U>, Resource {
        final long id;
