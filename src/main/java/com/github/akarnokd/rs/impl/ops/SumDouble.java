 ...
 *
 */
public final class SumDouble implements Publisher<Double> {
    final Publisher<? extends Number> source;
    public SumDouble(Publisher<? extends Number> source) {
        this.source = source;
    }
    
    @Override
    public void subscribe(Subscriber<? super Double> s) {
        source.subscribe(new SumDoubleSubscriber(s));
    }
    
    static final class SumDoubleSubscriber extends AtomicBoolean implements Subscriber<Number>, Subscription {
        /** */
        private static final long serialVersionUID = -6186305055696101343L;
        
        final Subscriber<? super Double> actual;
        double sum;
        boolean hasValue;
